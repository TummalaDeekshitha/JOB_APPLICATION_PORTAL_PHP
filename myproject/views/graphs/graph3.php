...
<div class="row mt-3">
    <div class="col-md-5 p-5">
        <div class="card">
            <h5 class="card-header">Core Jobs Search Distribution</h5>
    ...
    <div class="col-md-5 p-5">
        <div class="card">
            <h5 class="card-header">Software Job Search Distribution</h5>
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="softwareNavbar">
                        <ul class="navbar-nav">
                            ...
                            <li class="nav-item">
                                <button class="btn nav-link" onclick="showSoftwareGraph('10days')" id="10daysSoftwareBtn">Last 10 Days (Software)</button>
                            </li>
                            <li class="nav-item">
                                <button class="btn nav-link" onclick="showSoftwareGraph('1month')" id="1monthSoftwareBtn">Last 1 Month (Software)</button>
                            </li>
                            <li class="nav-item">
                                <button class="btn nav-link" onclick="showSoftwareGraph('2months')" id="2monthsSoftwareBtn">Last 2 Months (Software)</button>
                            </li>
                            <li class="nav-item">
                                <button class="btn nav-link" onclick="showSoftwareGraph('3months')" id="3monthsSoftwareBtn">Last 3 Months (Software)</button>
                            </li>
                        </ul>
                    </div>
                </div>
...
<script>
    ...
    function showCoreGraph(timeRange) {
        fetch(`/myproject/graphs/graphThree?category=core&timerange=${timeRange}`)
        ...
        if (coreChart) {
            ...
            coreChart = new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Top Most searched Core Jobs',
                        data: data,
    ...
    function showSoftwareGraph(timeRange) {
        fetch(`/myproject/graphs/graphThree?category=software&timerange=${timeRange}`)
            .then(response => response.json())
            .then(data => {
                const labels = data.labels;
                const documentCounts = data.documentCounts;
        ...
        if (softwareChart) {
            ...
            softwareChart = new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Top Most Searched Software ',
                        data: data,
