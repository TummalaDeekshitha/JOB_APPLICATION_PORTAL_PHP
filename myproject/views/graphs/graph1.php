...
<div class="row mt-3 ">
    <div class="col-md-5 p-5">
        <div class="card">
            <h5 class="card-header">Core Job Posts</h5>
    ...
    <div class="col-md-5 p-5">
        <div class="card">
            <h5 class="card-header">Software Job Posts</h5>
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
...
function showCoreGraph(timeRange) {
    fetch(`/myproject/graphs/graphOne?category=core&timerange=${timeRange}`)
        .then(response => response.json())
        .then(data => {
            const labels = data.labels;
            const documentCounts = data.documentCounts;
            displayCoreGraph(labels, documentCounts); // Corrected function name
        })
        .catch(error => console.error('Error fetching Core data:'));

    resetButtonStyles();
    document.getElementById(`${timeRange}CoreBtn`).classList.add('active');
}

function displayCoreGraph(labels, data) { // Corrected function name
    ...
    if (coreChart) {
        ...
        coreChart = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Number of Core Job Posted',
                    data: data,
...
function showSoftwareGraph(timeRange) {
    fetch(`/myproject/graphs/graphOne?category=software&timerange=${timeRange}`)
        .then(response => response.json())
        .then(data => {
            const labels = data.labels;
            const documentCounts = data.documentCounts;
        ...
        .catch(error => console.error('Error fetching Software data:'));
    ...
    if (softwareChart) {
        ...
        softwareChart = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Number of Software Posted',
                    data: data,
...
document.addEventListener('DOMContentLoaded', function () {
    // Call both functions after DOM content has loaded
    showCoreGraph("5days");
